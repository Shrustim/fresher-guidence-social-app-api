...
router.get('/byuserId/:userId', async(req, res) => {
 ...
 router.post('/searchpost', async(req, res) => {
   ...
   const querySql = "SELECT DISTINCT p.id , p.userId , p.postTitle , p.image , p.description , p.createdDate , p.updatedDate , p.is_active, tlike.totalLike, tComent.totalComments, pll.isLike, u.name, u.photo FROM posts p LEFT JOIN ( SELECT u.id, u.name,u.photo FROM users u LEFT OUTER JOIN user_friends f ON u.id = f.friendId or u.id = f.userId WHERE ( f.userId = '"+id+"' OR f.friendId = '"+id+"' ) AND f.isRequest = 0) u ON p.userId = u.id LEFT OUTER JOIN posts_likes pll ON pll.likeUserId = '"+id+"' AND p.id = pll.postId LEFT JOIN( SELECT COUNT(pl.id) AS totalLike, pl.postId FROM posts_likes pl WHERE pl.isLike = 1 AND pl.is_active = 1 GROUP BY pl.postId ) tlike ON tlike.postId = p.id LEFT JOIN( SELECT COUNT(pc.id) AS totalComments, pc.postId FROM posts_comments pc WHERE pc.is_active = 1 GROUP BY pc.postId ) tComent ON tComent.postId = p.id WHERE u.id IS NOT NULL and ( p.postTitle LIKE '%"+search+"%' OR p.description LIKE '%"+search+"%' ) ORDER BY p.id DESC";
    const rows = await connection({ querys: querySql, values: [] });
    // console.log("jjjjjj---------",rows)
    res.send(rows)    
 });
